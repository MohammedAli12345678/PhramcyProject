            ...
            using (var db = new DataBaseConnection())
            {
                ...
                foreach(var med in medicines)
                {
                    if (//this.searchMed.Text.Contains (med.name)
                        med.name.ToLower().Contains(searchText)
                        || (med.generic_name.ToLower().Contains(searchText))
                        || (med.category.name.ToLower().Contains(searchText))
                        || (med.Type.name.ToLower().Contains(searchText))
                        || med.company.name.ToLower().Contains(searchText))
